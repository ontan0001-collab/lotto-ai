...
def fetch_naver_lotto(drw_no=None):
    """네이버 검색 결과에서 로또 번호를 가져오는 가장 안정적인 방식"""
    try:
        if drw_no:
            url = f"https://search.naver.com/search.naver?query={drw_no}회+로또"
        else:
        ...
        return {"회차": "데이터 오류", "번호": [1,2,3,4,5,6], "보너스": 7}
...
def save_to_db(agent, nums):
    ...
    if not os.path.isfile(DB_FILE):
        df.to_csv(DB_FILE, index=False, mode='w', encoding='utf-8-sig')
    else:
        df.to_csv(DB_FILE, index=False, mode='a', header=False, encoding='utf-8-sig')
...
with tabs[0]:
    ...
    if st.session_state.current_draw:
        ...
        st.link_button("💬 결과 공유하기 (텔레그램/메시지)", f"https://t.me/share/url?url=LumenQuant&text={urllib.parse.quote(share_msg)}")

# --- Tab 2: 당첨번호 확인 (1회~최신) ---
with tabs[1]:
    st.header("🔍 전회차 당첨 조회 (네이버 엔진)")
    search_round = st.number_input("조회할 회차 입력", min_value=1, max_value=2000, value=1112)
    if st.button("데이터 조회"):
        data = fetch_naver_lotto(search_round)
        st.markdown(f"""
